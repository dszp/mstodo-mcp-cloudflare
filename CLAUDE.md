...
- `src/graph/substrate-client.ts` (`SubstrateClient`) — `substrate.office.com` (Exchange Online audience), the undocumented endpoint the To Do web app uses. Required because **Graph's `todoTask` has no `CommittedDay` field — My Day is invisible to Graph**. Substrate uses PascalCase Outlook shapes. Also powers **lossless cross-list move** via `reparentTask()` (PATCH `ParentFolderId`), which preserves checklist items / attachments / linked resources / My Day that a Graph create+delete would lose. See `src/mcp/move-task.ts` for the three-stage safety net (reparent → confirm `ParentFolderId` → only fall back to copy/delete if the source is confirmed still present). Also powers **checklist-item (subtask / "step") manual ordering** — Graph's `checklistItem` has no order field, so order is Substrate-only via `OrderDateTime` (same descending semantics as a task's). Read–write asymmetry, both verified live: the steps are **read INLINE** as a `Subtasks[]` array on the folder-scoped task GET (`listSubtasks(folderId, taskId)` does `GET /taskfolders/{folder}/tasks/{id}?$select=*` — there is **no** standalone subtasks collection; `GET /tasks/{id}/subtasks` 400s "Endpoint not supported"), but the **write is folder-free** per-item (`patchSubtask(taskId, subtaskId)` → `PATCH /tasks/{id}/subtasks/{subtaskId}`). A step's `OrderDateTime` is **null until it is first dragged** — un-reordered steps have no stored order and display in `createdDateTime` order. This is why **checklist order is never cached** (see the checklist-cache section): caching would mostly store nulls and add a per-scan Substrate call, while Graph's `createdDateTime` is the free, correct fallback. Backs the `reorder_checklist_item` tool and the live-order enrichment on `list_checklist_items` + `get_task` `include_checklist_order`.
